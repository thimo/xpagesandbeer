...
function dbLookupArray(server, dbname, viewname, keyname, field, params) {
  ...
  if (!!params && !!params.cache && params.cache != "false") {
    ...
    if (!!result && !!timestamp) {
    	if (timestamp < ((new Date().getTime()) - (TIMEOUT_LOOKUP_CACHE))) {
  ...
  if (!result) {
    ...
    if (!!result && !!params) {
	    ...
	    if (!!params.cache && params.cache != "false") {
	    	...
	    	sessionScope.put(cachekey + "_timestamp", (new Date().getTime()));
	    }
    }
  }
  return result;
...
function dbColumnArray(server, dbname, viewname, column, params) {
  ...
  if (!result) {
    ...
    if (!!result && !!params) {
	    ...
	    if (!!params.cache && params.cache != "false") {
	    	sessionScope.put(cachekey, result);
	    	sessionScope.put(cachekey + "_date", (new Date().getTime()));
	    }
  	}
  }
  return result;
